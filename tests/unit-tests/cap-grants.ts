...
test("basic cap grant", async t => {
  await runScenario(async (scenario: Scenario) => {
    ...
    const aliceCell = aliceHapp.cells.find(c => c.role_id === 'test')
    const bobCell = bobHapp.cells.find(c => c.role_id === 'test')

    const expected_private_function_result = "this is the result of the private function"
    ...
    t.deepEqual(first_private_function_result, expected_private_function_result)
    ...
    try {
      ...
      t.match(e.data.data, /^No capabilities grant has been committed that allows the CapSecret/)
    }
    ...
    t.deepEqual(second_private_function_result, expected_private_function_result)
  })
})
